...
class MainActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private fun runObjectDetection(bitmap: Bitmap) {
        /*create ML kit InputImage object == This converts the Bitmap into a format that ML Kit can process.*/
        val image = InputImage.fromBitmap(bitmap,0)
        /*Next, create an instance of the object detector. ML Kit uses a builder pattern to create the object detector, where you can set various options like enabling multiple objects and object classification.*/
        val  options = ObjectDetectorOptions.Builder()
            .setDetectorMode(ObjectDetectorOptions.SINGLE_IMAGE_MODE)// Single image mode
        ...
        //Feed the Image to the Detector
        objectDetector.process(image)
            .addOnSuccessListener { detectedObjects ->
                // Task completed successfully
                debugPrint(detectedObjects) //// Call a helper function to log the detected objects
                // Parse ML Kit's DetectedObject and create corresponding visualization data
                val parseDetectedobjects = detectedObjects.map{obj ->
                    var text = "Unknown"
...
// Draw the detection result on the input bitmap
                val visualizedResult = drawDetectionResult(bitmap, parseDetectedobjects)
